...
export default function ReceptionSettingsTab() {
  ...
  useEffect(() => {
    try {
      const savedStations = (typeof window !== "undefined" ? localStorage.getItem(STORAGE_KEYS.STATIONS) : null);
      if (savedStations) setStations(JSON.parse(savedStations));

      const savedDevices = (typeof window !== "undefined" ? localStorage.getItem(STORAGE_KEYS.DEVICES) : null);
      if (savedDevices) setDevices(JSON.parse(savedDevices));

      const savedConfig = (typeof window !== "undefined" ? localStorage.getItem(STORAGE_KEYS.CONFIG) : null);
